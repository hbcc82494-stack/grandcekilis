...
// default hash for "ChangeMe!2025"
const DEFAULT_HASH = '$2a$10$8r1GxXH6Q1m1Qk2o/b0m3uI2qUX1k4nJ5F1eYpA0seI6ex1x5gM7G'; 
...
app.get('/admin', requireAuth, async (req,res)=>{
  const draws = await db.all(`SELECT id,title,created_at,winners_count FROM draws ORDER BY created_at DESC LIMIT 50`);
  const body = `
  <div class="card">
...
app.post('/admin/create', requireAuth, async (req,res)=>{
  try{
    ...
    if(!Array.isArray(names) || names.length === 0) throw new Error('İsim listesi boş olamaz');
    ...
    shuffle(amtPool);
    const final = winners.map((n,i)=>({ name:n, amount: amtPool[i % amtPool.length] }));

    const id = nanoid(8);
    const created_at = new Date().toISOString().replace('T',' ').slice(0,19);
    await db.run(`INSERT INTO draws (id,title,created_at,winners_count,result_json) VALUES (?,?,?,?,?)`,
      [id, title, created_at, winnersCount, JSON.stringify(final)]);
    await db.run(`INSERT INTO audit(draw_id, action, ts) VALUES (?,?,?)`, [id,'create', created_at]);

    res.redirect('/r/'+id);
  }catch(e){
    res.status(400).send(layout('Hata', `<div class="alert">Hata: ${e.message}</div><a href="/admin">Geri dön</a>`));
  }
});
  ...
  <script>
    ...
    function reveal(){
      rowsEl.innerHTML = winners.map((w,i)=>\`
        <tr><td>\${i+1}</td><td>\${escapeHtml(w.name)}</td><td>\${escapeHtml(String(w.amount))} ₺</td></tr>
      \`).join('');
